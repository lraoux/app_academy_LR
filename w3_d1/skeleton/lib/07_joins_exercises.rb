    ...
    FROM
      (SELECT
       ...
       JOIN
        actors ON castings.actor_id = actors.id
       WHERE
        actors.name = 'Julie Andrews') AS ja
    JOIN
      castings ON ja.id = castings.movie_id
    JOIN
      actors ON castings.actor_id = actors.id
    WHERE
      castings.ord = 1;
    ...
    FROM
      (SELECT
       ...
       JOIN
        actors ON castings.actor_id = actors.id
       WHERE
        actors.name = 'Art Garfunkel') AS ag
    JOIN
      castings ON ag.id = castings.movie_id
    JOIN
      actors ON castings.actor_id = actors.id
    WHERE
